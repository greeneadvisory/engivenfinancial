...
const toNumber = (value: CryptoValue) => {
...
const getYearFromRow = (row: CryptoRow) => {
...
const getSummaryIdentity = (row: CryptoRow) => {
...
const CryptoTransOrgSummaryDataTable = () => {
  const [data, setData] = React.useState<CryptoRow[]>([]);
  ...
  const yearFilteredData = React.useMemo(() => {
    ...
    return data.filter((row) => getYearFromRow(row) === selectedYear);
  }, [data, selectedYear]);
  ...
  const summaryData = React.useMemo(() => {
    ...
    yearFilteredData.forEach((row) => {
      ...
      const location = npoLocations[buildNpoLookupKey(npoName, ein)] ?? { city: "--", state: "--" };
  ...
  const columns = React.useMemo<any[]>(
    () => [
      ...
      },
      {
        ...
        selector: (row: SummaryRow) => row.donationCount,
        sortable: true,
        sortFunction: (a: SummaryRow, b: SummaryRow) => a.donationCount - b.donationCount,
        field: "donationCount",
      },
      {
        name: "Total $ Donations",
  ...
  const exportFilteredAsCsv = React.useCallback(() => {
    ...
    link.href = url;
    link.setAttribute("download", `crypto-trans-org-summary-${yearSegment}-${timestamp}.csv`);
  ...
  const bottomTotals = React.useMemo(() => {
    return summaryData.reduce(
      (accumulator, row) => ({
        donationCount: accumulator.donationCount + row.donationCount,
      ...
      <div className="mt-3 rounded-sm border border-defaultborder bg-defaultbackground/30 px-3 py-2 text-[0.8125rem]">
        <div className="grid grid-cols-12 gap-3 items-center">
          ...
          <div className="col-span-12 md:col-span-2 text-end tabular-nums">
            <span className="text-textmuted me-1">Donations:</span>
            <strong>{bottomTotals.donationCount}</strong>
          </div>
          <div className="col-span-12 md:col-span-2 text-end tabular-nums">
...
export default CryptoTransOrgSummaryDataTable;
