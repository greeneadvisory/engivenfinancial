...
const Sidebar = ({ local_varaiable, ThemeChanger }: any) => {
	...
	const setPendingUpdatesLabels = React.useCallback((npoPendingCount: number, cryptoPendingCount: number) => {
		const applyLabel = (items: any[]): any[] => {
			return items.map((item: any) => {
				...
				if (Array.isArray(updatedItem.children)) {
					updatedItem.children = applyLabel(updatedItem.children);
				}
		...
		setMenuitems((previous: any) => applyLabel(previous));
	}, []);
	...
	useEffect(() => {
		...
		const idleCallback =
			typeof window !== "undefined" && "requestIdleCallback" in window
				? window.requestIdleCallback(scheduleInitialRefresh)
				: window.setTimeout(scheduleInitialRefresh, 500);
		...
		const handleExternalRefresh = () => {
			refreshPendingUpdatesCount(true);
		};
		...
		<Fragment>
			...
			<aside ref={sidebarRef} className={`app-sidebar ${isSticky ? "sticky-pin" : ""}`} onMouseOver={() => Onhover()} onMouseLeave={() => Outhover()} id="sidebar">
				<div className="main-sidebar-header">
					<Link href="/dashboard/sales" className="header-logo" onMouseEnter={() => prefetchRoute('/dashboard/sales')}>
									...
									<li  ref={slideRef} className={`${levelone.menutitle ? 'slide__category' : ''} ${levelone.type === 'link' ? 'slide' : ''}
										...
										{levelone.type === "link" ?
											<Link
												...
												onMouseEnter={() => prefetchRoute(levelone.path)}
											>
												<span className={`${local_varaiable?.dataVerticalStyle == 'doublemenu' ? '' : 'd-none'}`}>
													...
													<span className={`hs-tooltip inline-block [--placement:right] leading-none ${local_varaiable?.dataVerticalStyle == 'doublemenu' ? '' : 'hidden'}`}>
														<SpkButton buttontype="button" customClass="hs-tooltip-toggle  inline-flex justify-center items-center">
															{showSidebarIconsOnlyOnToggle ? levelone.icon : ""}
												...
												{local_varaiable.dataVerticalStyle != "doublemenu" && showSidebarIconsOnlyOnToggle ? levelone.icon : ""}
