import { NpoRecord } from "@/shared/lib/npo-sync";

const NPO_REPORT_URL = "https://platform.engiven.com/api/admin/reports/npo";
...
export async function fetchEngivenNpos() {
  ...
  const response = await fetch(NPO_REPORT_URL, {
    method: "POST",
    headers: {
  ...
  if (!response.ok) {
    const fallbackMessage = rawText
      ...
      : "Failed to fetch NPO report data.";
